...
namespace DBHelper
{
    public partial class SqlHelper : BaseHelper
    {
        ...
        /// <returns></returns>
        public static string[] GetDatabases(string connectionString=null)
        {
            string sql = "select name from sys.databases where name not in ('master','model','msdb','tempdb')"; 
            DataTable dt = ExecuteDataTable(sql, connectionString);
        ...
        /// <returns></returns>
        public static List<DbTable> GetDbTables(string database, string connectionString = null)
        {
            #region SQL
            string sql = string.Format(@"SELECT
                                        ...
                                        (
	                                        CASE 
		                                        WHEN (SELECT COUNT(1) FROM sys.indexes WHERE object_id= obj.OBJECT_ID AND is_primary_key=1) >=1 THEN 0
		                                        ELSE 1
	                                        END 
